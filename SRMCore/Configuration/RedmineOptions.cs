...
public class RedmineOptions
{
    ...
    public int IssueRefreshIntervalSeconds { get; set; } = 15;
    public int WarningPriorityId { get; set; } = 3;
    public int MajorPriorityId { get; set; } = 4;
    public int CriticalPriorityId { get; set; } = 5;

    public string BuildPublicIssueUrl(string issueId)
    {
        var publicBaseUrl = string.IsNullOrWhiteSpace(PublicBaseUrl) ? BaseUrl : PublicBaseUrl;
        return $"{publicBaseUrl.TrimEnd('/')}/issues/{issueId}";
    }
}
